...
for degree = [1e-6 1e-5 0.0001  0.001  0.01 0.1]
E1 = randn(M,N);E2 =randn(M,N);
E3 =randn(M,N);E4 = randn(M,N);
E = degree * quaternion(E1,E2,E3,E4);
Ahat = A +   E;
% A = randq(M,N);
K =50; step = 10;
normE = [normE, norm(E)];
...
tStart = tic;
 ...
 cc = ceil(k * log(k)) ; rr = ceil(k * log(k)) ;
  normX = norm(Ahat(:))^2;
    %% Update L = C * pinv_U * R
  [C,cols ] = col_length(Ahat,cc,normX);
  [R,rows] = row_length(Ahat,rr,normX);
...
% Ahat = A +  degree * E; k = 50;
 ...
 imname='Permutation_noise_length';
